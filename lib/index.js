...
PastyClient.prototype.__version = "0.3.0-master";
 ...
 * @param {function} [callback] Function that will receive {object} [E] Error Object and {object} [payload] containing username and availability.
 * @return {null}
 * @api public
 */
...
PastyClient.prototype.HTTPRequest = function(target, method, authData, data, callback) {
  ...
  var resHandler = function(res) {
    ...
    res.on('end', function() {
      ...
      if(code >= 200 && code <= 206) { // request was successful
        var answer = JSON.parse(data); // Todo Catch exception
        ...
        var answer = JSON.parse(data);  // Todo Catch exception
        console.log(typeof(answer.error));
  ...
  req.on('error', function (error) {
    switch (error.code) {
      case "ECONNREFUSED":
        var E = new Object({"http_code": 500, "message": "Remote server refused the connection. Please check settings."});
        break;
      case "ECONNRESET":
        var E = new Object({"http_code": 500, "message": "Connection to remote server was resetted. Please try again."});
        break;
      case "ETIMEDOUT":
        var E = new Object({"http_code": 500, "message": "Connection to remote server timed out. Please check settings."});
        break;
      default:
        if(error.toString().search("error:140770FC:SSL") !== -1) {
          var E = new Object({"http_code": 500, "message": "Seems like remote server does not speak SSL."});
        } else {
          var E = new Object({"http_code": 500, "message": "Unknown error occured."});
        }
        break;
    }
    callback(E, null);
